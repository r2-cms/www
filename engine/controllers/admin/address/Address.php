<?php
	...
	class Address extends CardLister {
		public $name		= 'address';
		public $root		= '';
		public $estados		= '';
		public $cidades		= '';
		...
		function __construct() {
			...
			***************************************************************************/
			...
			for( $i=0; $i<count($this->estados); $i++) {
				$this->estados[$i]	= RegExp($this->estados[$i], '[A-Za-z]{2}');
			}
		...
		private function buildHTML() {
			...
			$this->addToolbarItem('Adicionar endereço', 'add-new-address', CROOT.$GT8['admin']['root'].'address/new-address/', CROOT.'imgs/gt8/toolbar/file-add-small.png');
			...
			$this->addSideBarItem( 'Filtros', 'filters', (CROOT.'imgs/gt8/filter-small.png'), '
				<label title="Busca por CEP" ><span><input type="text" value="'. (isset($_GET['zip'])? ((($_GET['zip']))): '') .'" name="zip" class="gt8-update input-rounded-shadowed" onkeyup="Pager.search(this, event)" /><small>Busca por cep</small></span></label>
				<label title="Busca por estado" ><span><input type="text" value="'. (isset($_GET['uf'])? ((($_GET['uf']))): '') .'" name="uf" class="gt8-update input-rounded-shadowed" onkeyup="Pager.search(this, event)" /><small>Busca por estados</small></span></label>
				<label title="Busca por cidade" ><span><input type="text" value="'. (isset($_GET['city'])? ((($_GET['city']))): '') .'" name="city" class="gt8-update input-rounded-shadowed" onkeyup="Pager.search(this, event)" /><small>Busca por cidades</small></span></label>
				<label title="Procurar por ruas" ><span><input type="text" value="'. (isset($_GET['street'])? ((($_GET['street']))): '') .'" name="street" class="gt8-update input-rounded-shadowed" onkeyup="Pager.search(this, event)" /><small>Procurar por ruas</small></span></label>
			');
		...
		private function checkActionRequest() {
			if ( $_GET['opt'] == 'update') {
				...
				require_once( SROOT .'engine/queries/address/deleteAddress.php');
				deleteAddress($_GET);
				die();
