<?php
	...
	class AdminEditor extends Editor {
		public $name	= 'offers-config';
		public $tableType	= null;
		
		function __construct() {
			global $GT8, $spath;

			$page	= $this->getSPath('offers-config');
		...
		public function setCards() {
			...
			$this->options['addSelect']	.= ', SUBSTRING(SUBSTRING_INDEX(e.path, "/", 4), 10) AS l_path';
		...
		private function checkActionRequest() {
			if ( isset($_GET['action']) && $_GET['action']) {
				switch($_GET['action']) {
					case 'save-position': {
						require_once( SROOT .'engine/queries/offers-config/SaveSettings.php');
