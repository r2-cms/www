...
jCube.Array.each=(function(jCube){if(jCube.config.alterNativePrototype){Array.prototype.each=function(){return jCube.Array.each.apply(this,arguments)}}return(function(callback){for(var i=0,len=this.length;i<len;i++){callback.call(this[i],this[i],i,this)}return this})})(jCube);jCube.Include.files["Array.hasDuplicates"];
...
jCube.Array.removeDuplicates=Array.prototype.removeDuplicates=function(){var copy=[].concat(this),i=0;while((i=copy.hasDuplicates())){copy.del(i)}return copy};jCube.Include("Array.del");jCube.Include("Array.hasDuplicates");jCube.Include.files.Browser;
...
jCube.Element.removeClass=(function(jCube){jCube.Include("Array.remove");jCube.Include("String.trim");return jCube.Element.Implements({name:"removeClass",method:function(name){this.className=this.className.split(" ").remove(name).join(" ").trim();return this}})})(jCube);jCube.Include.files["Element.setBackgroundPosition"];
...
jCube.Number.add=Number.prototype.add=function(n){return this+n};jCube.Include.files["Number.check"];
jCube.Number.check=Number.prototype.check=function(alt){if(isFinite(this)&&!isNaN(this)){return Number(this)}return alt};jCube.Include.files["Number.format"];
...
jCube.Object.each=(function(jCube){if(jCube.config.alterNativePrototype){Object.prototype.each=function(){return jCube.Object.each.apply(this,arguments)}}return function(callback,bind){if(callback==null){throw new Error("missing argument 0 when calling Object.prototype.map")}else{if(typeof callback!=="function"){throw new Error('"callback" parameter must be a function!')}}for(var i in this){if(Object.prototype[i]!=this[i]){callback.call(bind||this,i,this[i],this)}}return this}})(jCube);jCube.Include.files["Pluggins.BackgroundEffect"];
jCube.Include("Element.addClass");jCube.Include("Element.getElementsBySelector");jCube.Include("Element.getNodeIndex");jCube.Include("Element.getParent");jCube.Include("Element.removeClass");jCube.Include("Element.setStyle");jCube.Include("Event.add");jCube.Include("Transition.fadeIn");jCube.Include("UI.BackgroundEffect");jCube.Pluggins.BackgroundEffect=function(options){var main=options.container;onStart=options.onStart||options.onBackgroundChangeStart;onChange=options.onChange||options.onBackgroundChange;onComplete=options.onComplete||options.onBackgroundChangeComplete;var be=new jCube.UI.BackgroundEffect(options.merge({container:":.images-step",onStart:function(){var index=this.getIndex();var titles=main.query("::.titles-step h5");var texts=main.query("::.texts-step p");var steps=main.query("::.slider-steps a");titles[index].fadeOut({duration:500,onComplete:function(){titles.removeClass("active")[index].setStyle("opacity",0).addClass("active").fadeIn(2000)}});texts[index].fadeOut({duration:500,onComplete:function(){texts.removeClass("active")[index].setStyle("opacity",0).addClass("active").fadeIn(2000)}});steps.removeClass("active")[index].addClass("active");if(onStart){onStart.call(this)}},onChange:function(){if(onChange){onChange.call(this)}},onComplete:function(){if(onComplete){onComplete.call(this)}}}));be.container.getParent().getParent().query("::.slider-steps a").addEvent("onclick",function(E){E.stop();be.goToIndex(main.query("::.slider-steps a").length-this.getNodeIndex()-1)});var __start=be.start;be.start=function(i){__start.call(be,i);jCube("::.lazy-load div").each(function(){be.addImage(this.innerHTML,this.title,null,this.className)});be.start=__start;return be};return be};jCube.Include.files["Pluggins.MenuDropDown"];
jCube.Pluggins.MenuDropDown=(function(jCube){jCube.Include("Element.addClass");jCube.Include("Element.appendTo");jCube.Include("Element.getElementsBySelector");jCube.Include("Element.getLastSibling");jCube.Include("Element.getOffset");jCube.Include("Element.getParent");jCube.Include("Element.getPreviousSibling");jCube.Include("Element.remove");jCube.Include("Element.removeClass");jCube.Include("Element.setPositionByCoordinates");jCube.Include("Element.setStyle");jCube.Include("Event.add");jCube.Include("Function.delay");jCube.Include("String.left");jCube.Include("Time.Chronometer");jCube.Include("Transition.moveTo");jCube.Include("Pluggins/MenuDropDown/MenuDropDown.css");return(function enclosure(options){options=({showDelay:30,showDuration:600,hideDelay:600,hideDuration:500,offsetY:2,hideWait:50}).merge(options||{});options.container=jCube(options.container);var index=1000;options.container.query("::.top").each(function(){var eSub,eUl,eAba;eSub=this.query(":..MenuDropDown-sub");if(eSub){eUl=eSub.query(":.holder")||eSub.query(":ul");eAba=eSub.getPreviousSibling();eSub.setStyle({width:eUl.offsetWidth+10});var showing=false;eAba.addEvent("onmouseover",function(e){if(!showing){if(eSub.parentNode!=document.body){var bounds=eAba.getOffset(document.body);eSub.appendTo(document.body).setStyle("width","auto");eSub.setStyle("width",eUl.offsetWidth+10);eUl.setStyle({top:(eUl.offsetHeight+40)*-1});eSub.setPositionByCoordinates({clientX:bounds.left,clientY:bounds.bottom})}eUl.setStyle("left",0).moveTo({onStart:function(){showing=true},onComplete:function(){showing=false},left:0,top:options.offsetY,delay:options.showDelay,duration:options.showDuration}).getParent().setStyle("zIndex",index++);this.addClass("over")}chronOut.pause()}).addEvent("onmouseout",function(e){chronOut.start(options.hideDelay)});var chronOut=new jCube.Time.Chronometer({onComplete:function(){showing=false;eUl.moveTo({left:0,top:(eUl.offsetHeight+40)*-1,duration:options.hideDuration,onComplete:function(){eAba.removeClass("over");showing=false;eSub.remove()}})}});eUl.query("::li").addEvent("onmouseover",function(e){eAba.trigger("onmouseover",e)}).addEvent("onmouseout",function(e){eAba.trigger("onmouseout",e)});eSub.remove()}})})})(jCube);jCube.Include.files["Pluggins.Slider"];
jCube.Include("UI.Slider");jCube.Include("Pluggins/Slider/themes/"+jCube.config.theme+"/main.css");jCube.Pluggins.merge({Slider:function(options){var Slider=(function(options){var slider=jCube(options.slider);var knob=slider.query(":.knob");var bar=slider.query(":.bar");knob.setStyle("position","absolute");this.setValue=function(v,p){return this.ancestor.setValue.call(this,v,p)};this.init(options);return this}).inherits(jCube.UI.Slider);jCube.Pluggins.Slider=Slider;return new Slider(options)}});jCube.Include.files["Pluggins.TabbedPane"];
...
jCube.Transition.resizeTo=(function(jCube){jCube.Include("Transition");return jCube.Element.Implements({name:"resizeTo",disallow:["BODY","WINDOW"],method:function(width,height,duration,transition,onComplete){var props=width;if((width!=null&&typeof width!="object")||(height!=null&&typeof height!="object")){props={width:width,height:height,duration:duration,transition:transition,onComplete:onComplete}}var css=[],dest=[];if(props.width!=null){css.push("width");dest.push(props.width||0)}if(props.height!=null){css.push("height");dest.push(props.height||0)}return jCube.Transition.Implements(props.merge({obj:this,name:"resizeTo",css:css,dest:dest,_onStart:"onResizeStart",_onChange:"onResize",_onComplete:"onResizeComplete"}))}})})(jCube);jCube.Include.files["UI.BackgroundEffect"];
jCube.UI.BackgroundEffect=(function(jCube){jCube.Include("Element.addClass");jCube.Include("Element.appendTo");jCube.Include("Element.getData");jCube.Include("Element.getElementsBySelector");jCube.Include("Element.getFirstChild");jCube.Include("Element.getLastChild");jCube.Include("Element.getParent");jCube.Include("Element.getPreviousSibling");jCube.Include("Element.injectBefore");jCube.Include("Element.setData");jCube.Include("Element.setStyle");jCube.Include("Math.random");jCube.Include("Function.delay");jCube.Include("String.toInteger");jCube.Include("Time.Chronometer");jCube.Include("Transition");jCube.Include("Transition.fadeIn");return function(options){var duration=options.duration||800,wait=options.wait||10000,container=jCube(options.container),transition=options.transition||jCube.Transition.ACUTE,random=options.random||false;this.SBE=this;this.container=container;this.onBackgroundChangeStart=options.onStart||options.onBackgroundChangeStart;this.onBackgroundChange=options.onChange||options.onBackgroundChange;this.onBackgroundChangeComplete=options.onComplete||options.onBackgroundChangeComplete;this.onBackgroundTimeEllapsing=options.onTimeEllapsing||options.onBackgroundTimeEllapsing;this.duration=duration;this.wait=wait;this.transition=transition;this.random=random;this.fps=15;this.tellapsed=0;var index=-1;var crrImage=null;var BE=this;var chron=new jCube.Time.Chronometer({fps:10,onChange:function(){BE.tellapsed=this.time;if(BE.onBackgroundTimeEllapsing){BE.onBackgroundTimeEllapsing(BE.tellapsed)}if(container){container.trigger("onBackgroundTimeEllapsing",BE.tellapsed)}},onComplete:function(){BE.start()}});this.getChron=function(){return chron};this.stop=function(){if(this._timeout){window.clearTimeout(this._timeout)}chron.stop();this._stopped=true;return this};var __firstStep=true;this.pause=function(){chron.pause();this._stopped=true;return this};this.resume=function(){if(this.crrImg){chron.resume();return this}return this};this.goToIndex=function(index){return this.start(index)};this.start=function(index){if(this._timeout){window.clearTimeout(this._timeout)}if(glassPaneLocal.getPreviousSibling().getPreviousSibling()){glassPaneLocal.getPreviousSibling().getPreviousSibling().style.visibility="hidden";glassPaneLocal.getPreviousSibling().getPreviousSibling().style.zIndex=0;var crr;if(this.random){crr=this.container.childNodes[Math.random(0,this.container.childNodes.length-3)]}else{crr=this.container.getFirstChild()}this.crrImg=crr;crrImage=crr;if(!crr.style){throw new Error("Invalid object!")}crr.style.zIndex=1;crr.style.visibility="";crr.style.display="";crr.setStyle("opacity",0);crr.injectBefore(glassPaneLocal);if(index!=null&&crr.getData("BackgroundEffect::index")!=(crr.getParent().query("::.images-step > .img").length-index-1)){this.start(index);return null}crr.style.left=0+"px";crr.style.top=0+"px";crr.style.width=this.container.style.width.toInteger(this.container.offsetWidth)+"px";crr.style.width="100%";if(index!=null){}if(__firstStep){crr.setStyle("opacity",1);__firstStep=false}else{crr.fadeIn({duration:this.duration,transition:this.transition,fps:this.fps,onChange:function(){if(this.onFade){this.onFade()}if(BE.onBackgroundChange){BE.onBackgroundChange()}},onComplete:function(){if(this.onFadeEnd){this.onFadeEnd()}if(BE.onBackgroundChangeComplete){BE.onBackgroundChangeComplete()}}})}this.TransitionFade=crr.getData("transition::fadeIn");if(this.onBackgroundChangeStart){this.onBackgroundChangeStart()}}this._stopped=false;chron.start(this.wait);glassPaneLocal.style.width=this.container.offsetWidth+"px";glassPaneLocal.style.height=this.container.offsetHeight+"px";return this};this.addImage=function(src,href,onClick,className){var a=jCube(document.createElement("A"));var img=jCube(document.createElement("IMG"));a.href=href;a.style.position="absolute";a.style.zIndex=this.container.getFirstChild()?this.container.getFirstChild().style.zIndex:0;a.style.display="none";a.style.visibility="hidden";a.setStyle("opacity",0);img.container=this.container;a.container=this.container;a.SBE=this;img.SBE=this;var p=new Image();p.src=src;a.injectBefore(glassPaneLocal.getPreviousSibling()||glassPaneLocal);p.onload=function(){img._imgPreloaded=true};p.onerror=function(){a.parentNode.removeChild(a)};img.src=p.src;a.className="img";if(className){a.addClass(className)}if(onClick){a.onclick=onClick}a.setData("BackgroundEffect::index",++index);img.appendTo(a);return a};this.getIndex=function(){return crrImage.getData("BackgroundEffect::index")};(function(){var textNodes=[];for(var next=jCube(container).firstChild;next;next=next.nextSibling){if(next.nodeType==3){textNodes.push(next)}}for(var i=0;i<textNodes.length;i++){container.removeChild(textNodes[i])}if(container.hasChildNodes()){for(var next=jCube(container).firstChild;next;next=next.nextSibling){jCube(next);next.style.display="none";next.setStyle("opacity",0);next.style.position="absolute";next.style.zIndex=0;next.container=container;next.SBE=this;if(!next.onload){next.onload=function(){if(this.getFirstChild()){this.getFirstChild()._imgPreloaded=true}else{this._imgPreloaded=true}}}if(!next.onerror){next.onerror=function(){this.parentNode.removeChild(this)}}next.setData("BackgroundEffect::index",++index)}if(container.getLastChild().getPreviousSibling()){container.getLastChild().getPreviousSibling().setStyle("display","").setStyle("opacity",1);container.getLastChild().setStyle("display","").setStyle("opacity",0)}else{container.getLastChild().setStyle("display","").setStyle("opacity",1)}}})();(function(){this.container.addEvent("onmouseover",function(E){chron.pause()}).addEvent("onmouseout",function(E){chron.resume()})}).call(this);var glassPaneLocal=jCube(document.createElement("DIV")).setStyle({position:"absolute",left:"0px",top:"0px",width:"100%",height:"100%",background:"#ffffff",zIndex:5,cursor:"default",opacity:0,display:"none"}).appendTo(container);return this}})(jCube);jCube.Include.files["UI.Slider"];
jCube.UI.Slider=(function(jCube){jCube.Include("Event.add");jCube.Include("Element.getComputedStyle");jCube.Include("Element.getData");jCube.Include("Element.getElementsBySelector");jCube.Include("Element.getFirstChild");jCube.Include("Element.getLastChild");jCube.Include("Element.remove");jCube.Include("Element.setDraggable");jCube.Include("Element.setData");jCube.Include("Element.setStyle");jCube.Include("Function.inherits");jCube.Include("Math.between");jCube.Include("Math.getLinearEquation");return jCube.Object.merge.call(function(options){this.setValue=function(value,percent){value=Number(value);var slider=this.eSlider;var bar=this.eBar;var knob=this.eKnob;if(!slider){}if(isNaN(value)){value=options.min}var width,height,s;if(this.orientation==jCube.UI.Slider.VERTICAL_ORIENTATION){s=slider.offsetHeight||bar.offsetHeight;height=s-knob.offsetHeight}else{s=slider.offsetWidth;width=s-knob.offsetWidth}var min=this.min,max=this.max,increment=this.increment,valueParsed;if(percent!=null){value=Parse(percent*100)}if(value>max){value=max}else{if(value<min){value=min}}var d=min*-1;min=0;max+=d;valueParsed=(((value+d)*100)/max);if(this.orientation==jCube.UI.Slider.VERTICAL_ORIENTATION){knob.setStyle("top",valueParsed/(100/height))}else{knob.setStyle("left",valueParsed/(100/width))}this.value=slider.value=value;var p=Math.getLinearEquation(this.min,0,this.max,1,value);if(this.onSlide){this.onSlide({},value,p)}if(slider.onSlide){slider.onSlide({},value,p)}slider.trigger("onSlide",value,p);return this};function Parse(value){var increment=self.increment,min=self.min,max=self.max,d=0,valueParsed=value;d=min*-1;min=0;max+=d;valueParsed=((valueParsed*max)/100)-d;var valueI=1;if(valueI<0){valueI=-1}var difDown=(valueParsed*valueI)%increment,difUp=difDown-increment;if(difDown>difUp*-1){valueParsed=(valueParsed*valueI)-difUp}else{valueParsed=(valueParsed*valueI)-difDown}return valueParsed}this.init=function(options){var slider=jCube(options.slider);var bar=slider.query(".bar")[0];var knob=slider.query(".knob")[0];this.eSlider=slider;this.eBar=bar;this.eKnob=knob;this.min=options.min==null?0:options.min;this.max=options.max==null?100:options.max;this.value=options.value==null?0:options.value;this.focusable=options.focusable||false;this.increment=options.increment||1;this.superIncrement=options.superIncrement||(this.max-this.min)/20;this.orientation=options.orientation||jCube.UI.Slider.HORIZONTAL_ORIENTATION;this.grid=options.grid||1;slider.value=this.value=options.value;slider.slideTo=function(value,percent){this.getData("UI.Slider").setValue(value,percent);return this};slider.setValue=slider.slideTo;if(options.onSlideStart||options.onStart){slider.addEvent("onSlideStart",options.onSlideStart||options.onStart)}if(options.onSlide){slider.addEvent("onSlide",options.onSlide)}if(options.onSlideComplete||options.onComplete){slider.addEvent("onSlideComplete",options.onSlideComplete||options.onComplete)}function DoEvents(e,x,y,vnt){var v=self.orientation==jCube.UI.Slider.VERTICAL_ORIENTATION?y:x;var eqL=Math.getLinearEquation(0,self.min,1,self.max,v);if(self[vnt]){self[vnt](e,eqL,v)}if(slider[vnt]){slider[vnt](e,eqL,v)}slider.value=eqL;self.value=eqL;slider.trigger(vnt,eqL,v)}knob.addEvent("onDragStart",function(e,x,y){OBJbar="knob";DoEvents(e,x,y,"onSlideStart")});knob.addEvent("onDrag",function(e,x,y){DoEvents(e,x,y,"onSlide")});knob.addEvent("onDragComplete",function(e,x,y){DoEvents(e,x,y,"onSlideComplete");OBJbar=null});var OBJbar=null;var self=this;function __SlideTimeOut(timeout){if(OBJbar=="knob"){OBJbar=null;return }if(OBJbar){if(OBJbar.ltr){if(OBJbar.v>OBJbar.between){OBJbar.v-=self.superIncrement;OBJbar.v=Math.max(OBJbar.between,OBJbar.v);self.setValue(OBJbar.v);setTimeout(__SlideTimeOut,timeout||50)}}else{if(OBJbar.v<OBJbar.between){OBJbar.v+=self.superIncrement;OBJbar.v=Math.min(OBJbar.between,OBJbar.v);self.setValue(OBJbar.v);setTimeout(__SlideTimeOut,timeout||50)}}return }}bar.addEvent("mousedown",function(e){if(OBJbar=="knob"){OBJbar=null;return }var classe=knob.className;if(self.focusable){knob.className=classe.substring(0,classe.lastIndexOf("-"))+"-focus"}var end,start;OBJbar={e:e,v:self.value,ltr:(self.orientation==jCube.UI.Slider.VERTICAL_ORIENTATION?e.layerY+knob.offsetHeight/2<knob.offsetTop:e.layerX+knob.offsetWidth/2<knob.offsetLeft),between:(self.orientation==jCube.UI.Slider.VERTICAL_ORIENTATION?Math.getLinearEquation(knob.offsetHeight/2,self.min,bar.offsetHeight-knob.offsetHeight/2,self.max,e.layerY):Math.getLinearEquation(knob.offsetWidth/2,self.min,bar.offsetWidth-knob.offsetWidth/2,self.max,e.layerX))};window.setTimeout(function(){__SlideTimeOut(250)},250);return });bar.addEvent("mouseup",function(e){OBJbar=null});slider.setData("UI.Slider",this);knob.setStyle("position","relative");jCube(slider).query(".knob").setDraggable({bounds:"parent",grid:[this.grid,this.grid]});window.setTimeout(function(){slider.setValue(slider.value)},50);return this};return this},{VERTICAL_ORIENTATION:"vertical",HORIZONTAL_ORIENTATION:"horizontal"})})(jCube);jCube.Include.files["UI.TabbedPane"];
